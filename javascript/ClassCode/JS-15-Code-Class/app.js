...
userTodos.filter((a)=> a.userId===1).forEach((b)=>console.log(b.title,b.completed))

//! c- Birinci kullanıcının (id si 1 olan) sadece bitirdiği işlerin listesi

userTodos.filter((a)=> a.userId===1 && a.completed==true).forEach((a)=>console.log(a.completed,a.title))
...
let products = [
  ...
  products.forEach(({price,title})=> console.log(price,title))
...
//! d- Firmamız home-decoration categorisine sahip ürünleri ayrı bir sitede yayınlama kararı aldığı için bu ürünleri portföyünden çıkartacak. Bu karategorideki ürünleri listeden çıkaralım.(kalici)

products=products.filter(({category})=> category!=="home-decoration")

console.log(products) //category si home decoration olanlar silinmis oldu 

//! Kullanıcının seçtiği ürünü (id si girilen ürünü) silelim  -delete
//id si girilen ürün 5 olsun
const yeniProducts=products.filter(({id})=> id!==5)

console.log(yeniProducts)
...
//! Eger sifirdan bir object in icini elimdeki dizideki verilerden bazilariyla doldurmak istiyorsak , öncesinde object bos oldugu icin dot notation ile (sayilar.category) yapamam. Bu yüzden square bracket [] kullanmaliyim.( sayilar[category] )
...
const newList=customerList.map(({name,address,phone})=>({name,address,phone}))
...
customerList.map(({name,phone})=>{
  if(name.startsWith("E")){
    pazList.push({name,phone})

    
  }else if(name.startsWith("C")){
    carList.push({name,phone})
   
  }


})
console.log(`Carsamba aranacaklar listesi:`,carList)
console.log(`Pazartesi aranacaklar listesi:`,pazList)
...
const neuList=customerList.map(({id, name, email})=>({id,name, email}))
...
neuList.map(({name,id,email})=>{
  if(id%2!=0){
    elekCihaz.push({name,id,email,mesaj: 'Yeni çıkan XXX elektronik cihazımızı mutlaka denemelisiniz '})
  }else if(id%2==0){
    kahveMak.push({name,id,email,  mesaj: ' Yeni Çıkan kahve yapma makinamızı deneyin.Memnun kalacaksınız'})
